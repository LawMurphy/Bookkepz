document.addEventListener('DOMContentLoaded', function () {
  ...
  if (addItemBtn && invoiceBody) {
    addItemBtn.addEventListener('click', () => {
      const firstServiceSelect = document.querySelector('.service-type');
      let serviceOptions = firstServiceSelect
        ? Array.from(firstServiceSelect.options)
            .map(opt => `<option value="${opt.value}" data-desc="${opt.dataset?.desc || ''}" data-price="${opt.dataset?.price || ''}">${opt.textContent}</option>`)
            .join('')
        : '<option value="">Select Service</option>';

      const newRow = document.createElement('tr');
      newRow.innerHTML = `
        <td><select class="service-type">${serviceOptions}</select></td>
  ...
  if (invoiceBody) {
    ...
    invoiceBody.addEventListener('input', updateTotals);
  }
